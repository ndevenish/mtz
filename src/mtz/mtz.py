...
# Dataset =
class Dataset(
    namedtuple("Dataset", ["id", "name", "project", "crystal", "cell", "wavelength"])
):
    def __repr__(self):
...
def _parse_header(stream):
    ...
    ncols, nrefl, nbatch = get_only(
        [x for x in header_records if x.keyword == "NCOL"]
    ).data
    ...
    while True:
        ...
        if postheader.keyword == "MTZHIST":
            history = [
                stream.read(80).decode("ascii").strip() for x in range(postheader.data)
            ]
    ...
    header_batch_entries = list(
        itertools.chain(*[x.data for x in header_records if x.keyword == "BATCH"])
    )
    if len(header_batch_entries) != len(batches):
        raise InconsistentHeaderError(
            "Number of batches does not match declared header batch index count"
        )
    ...
    for entry, batch in zip(header_batch_entries, batches):
        if not entry == batch.serial:
            raise InconsistentHeaderError(
                "Batch serial order does not match header batch serial order"
            )
    ...
    if ncols != len([x for x in header_records if x.keyword in ("COL", "COLUMN")]):
        raise InconsistentHeaderError(
            "Column entries in header do not match column count"
        )
...
def _extract_datasets(header_records):
    ...
    dataset_ids = {
        x.data[4] for x in header_records if x.keyword in ["COL", "COLUMN"]
    } | {
        x.data[0]
        for x in header_records
        if x.keyword in ["PROJECT", "CRYSTAL", "DATASET", "DCELL", "DWAVEL"]
    }
    ...
    for datasetID in dataset_ids:
        name = get_only_or(
            x
            for x in header_records
            if x.keyword == "DATASET" and x.data[0] == datasetID
        )
        project = get_only_or(
            x
            for x in header_records
            if x.keyword == "PROJECT" and x.data[0] == datasetID
        )
        crystal = get_only_or(
            x
            for x in header_records
            if x.keyword == "CRYSTAL" and x.data[0] == datasetID
        )
        cell = get_only_or(
            x for x in header_records if x.keyword == "DCELL" and x.data[0] == datasetID
        )
        wavelength = get_only_or(
            x
            for x in header_records
            if x.keyword == "DWAVEL" and x.data[0] == datasetID
        )
        datasets.append(
            Dataset(
                id=datasetID,
...
def _extract_columns(header_records, datasets):
    ...
    for name, coltype, minVal, maxVal, datasetID in column_data:
        source = get_only(
            x for x in header_records if x.keyword == "COLSRC" and x.data[0] == name
        )
...
class Header(object):
    def __init__(self, raw_records, history=[], batches=[]):
        ...
        raw_records = [
            ...
            if not x.keyword
            in {"NDIF", "PROJECT", "DCELL", "DATASET", "CRYSTAL", "DWAVEL"}
        ]
        raw_records = [
            x for x in raw_records if not x.keyword in {"COL", "COLUMN", "COLSRC"}
        ]
...
class MTZFile(object):
    ...
    def __str__(self):
        ...
        lines.append(
            "Batches:  {}".format(rangestring([x.serial for x in self.header.batches]))
        )
        lines.append(
            "Datasets: {}".format(", ".join(x.name for x in self.header.datasets))
        )
